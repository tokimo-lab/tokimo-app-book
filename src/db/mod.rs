...
pub async fn init_pool() -> anyhow::Result<DatabaseConnection> {
    ...
    let schema = tokimo_bus_cli::manifest::parse_app_schema(crate::MANIFEST)?.unwrap_or_else(|| "book".to_string());

    let sep = if base_url.contains('?') { '&' } else { '?' };
    let encoded = urlencoding::encode(&schema);
    let url =
        format!("{base_url}{sep}application_name=tokimo-app-book&options=-c%20search_path%3D%22{encoded}%22%2Cpublic");
